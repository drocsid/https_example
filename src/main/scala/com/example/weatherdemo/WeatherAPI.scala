...
object WeatherAPI {
  ...
  case class Context(wx: String, geo: String, unit: String, `@vocab`: String)
  ...
  case class Elevation(value: Double, unitCode: String)
  ...
  implicit val weatherAPIC3c: Codec[WeatherAPI.Forecast] = {
    ...
    implicit val c1: Codec[WeatherAPI.SimpleGeometry] =
      deriveConfiguredCodec[WeatherAPI.SimpleGeometry]
    implicit val c2: Codec[WeatherAPI.Geometry] =
      deriveConfiguredCodec[WeatherAPI.Geometry]
