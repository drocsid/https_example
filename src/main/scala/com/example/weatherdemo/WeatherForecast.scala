...
object WeatherForecast {
  ...
  implicit def weatherEntityDecoder[F[_] : Sync]: EntityDecoder[F, WeatherForecast] = jsonOf

  implicit val weatherEncoder: Encoder[WeatherForecast] = deriveEncoder[WeatherForecast]

  implicit def weatherEntityEncoder[F[_]: Applicative]: EntityEncoder[F, WeatherForecast] =
    jsonEncoderOf
}
